...
class TagLibCx extends TagLib {
    ...
    protected $tags   =  array(
		...
		'powerby'=>array('close'=>0),
		'article'=>array('attr'=>'id,name,field,limit,order,where,sql,key,mod','level'=>3), 
	    'game'=>array('attr'=>'id,name,field,limit,order,where,sql,key,mod','level'=>3),
	    'category' => array('attr'=>'id,name,field,limit,order,where,sql,key,mod','level'=>3)
        );
	...
	public function _for($attr, $content){
	 ...
	 public function  _game($attr,$content)
	 {
				...
				$result = !empty($tag['result'])?$tag['result']:'game'; //定义数据查询的结果存放变量
	 ...
	 public function _article($attr,$content)
			...
			public function _powerby($attr,$content)
			{
				$tag = $this->parseXmlAttr($attr, 'powerby');
				$conn=M('config');
				$a=$conn->query("SELECT sitename FROM `mygame_webconfig` WHERE `id` =1");
                $xss=$a['0']['sitelx'];
                return $xss;
			}
