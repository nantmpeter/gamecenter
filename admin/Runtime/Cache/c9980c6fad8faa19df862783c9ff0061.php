<?php if (!defined('THINK_PATH')) exit();?>            <h3 class="f14"><span class="switchs cu on" title="充值管理"></span>充值管理</h3>
            <ul>
               <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li id="_MP<?php echo ($vo["access_id"]); ?>" class="sub_menu">
                    ...
                    <a href="javascript:_MP('<?php echo ($v2["id"]); ?>','./index.php?m=Pay&a=<?php echo ($v2["module_alias"]); ?>');" hidefocus="true" style="outline:none;"><?php echo ($v2["category"]); ?></a></li><?php endforeach; endif; else: echo "" ;endif; ?>
             </ul>
