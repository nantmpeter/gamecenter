<?php if (!defined('THINK_PATH')) exit();?>            <h3 class="f14"><span class="switchs cu on" title="全局设置"></span>全局设置</h3>
            <ul>
               <?php if(is_array($data1)): $i = 0; $__LIST__ = $data1;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v2): $mod = ($i % 2 );++$i;?><li id="_MP<?php echo ($v2["id"]); ?>" class="sub_menu">
                <a href="javascript:_MP('<?php echo ($v2["id"]); ?>','./index.php?m=Global&a=<?php echo ($v2["module_alias"]); ?>');" hidefocus="true" style="outline:none;"><?php echo ($v2["category"]); ?></a></li><?php endforeach; endif; else: echo "" ;endif; ?>
